...
def get_weather():
    ...
    key = '912831b297efd3f150772c529ee55cf6' # прописать свой API ключ
...
root.title('Прогноз погоды') # название окна
root.geometry('300x250') # размеры окна
root.resizable(width=False, height=False)
# Создаем фрейм (область для размещения других объектов)
frame_top = Frame(root, bg='pink', bd=5)
frame_top.place(relx=0.15, rely=0.15, relwidth=0.7, relheight=0.25)
# Все то - же самое, но для второго фрейма
frame_bottom = Frame(root, bg='pink', bd=5)
frame_bottom.place(relx=0.05, rely=0.55, relwidth=0.9, relheight=0.1)
...
info = Label(frame_bottom, text=' На данный момент в городе: ', bg='pink', font=40)
info.pack()
